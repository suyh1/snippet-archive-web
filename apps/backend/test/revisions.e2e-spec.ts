...
describe('Workspace file revisions API (e2e)', () => {
  ...
  it('lists revisions and restores an older revision', async () => {
    ...
    const listRes = await request(app.getHttpServer()).get(
      `/api/workspaces/${workspace.id}/files/${fileId}/revisions`,
    )
    ...
    const restoreRes = await request(app.getHttpServer()).post(
      `/api/workspaces/${workspace.id}/files/${fileId}/revisions/${revisionToRestore}/restore`,
    )
    ...
    const fileRes = await request(app.getHttpServer()).get(
    ...
    const afterRestoreList = await request(app.getHttpServer()).get(
      `/api/workspaces/${workspace.id}/files/${fileId}/revisions`,
    )
  ...
  it('returns 404 when revision does not belong to target workspace file', async () => {
    ...
    const revisionsA = await request(app.getHttpServer()).get(
      `/api/workspaces/${workspaceA.id}/files/${fileA.body.data.id}/revisions`,
    )
    ...
    const restoreRes = await request(app.getHttpServer()).post(
      `/api/workspaces/${workspaceB.id}/files/${fileB.body.data.id}/revisions/${revisionId}/restore`,
    )
