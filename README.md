...
Snippet Archive 的 Web 版本，当前已完成阶段二（团队协作升级）并交付阶段三首批能力（VS Code 插件 PoC + CLI v1）：
...
- `apps/vscode-extension`: VS Code 插件（保存/检索/插入片段）
- `apps/cli`: CLI 工具（检索、导出、导入、批量同步）
- `docs/openapi/workspace-v1.yaml`: 当前 API 契约文档（阶段二对齐）
- `docs/schemas/snippet-bundle.v1.schema.json`: 导入导出协议 schema（`snippet-archive-bundle/v1`）
...
## 阶段三工具说明（CLI + VS Code 插件）

### 各自作用

1. VS Code 插件（`apps/vscode-extension`）  
   在 IDE 内直接完成“登录 -> 保存片段 -> 搜索片段 -> 插入代码/跳转 Web”，减少来回切换浏览器的成本。
2. CLI（`apps/cli`）  
   面向自动化与批量场景，支持检索、导出、导入、同步，适合本地脚本或 CI 流程。
...
2. 登录（会将 token 写入 `~/.snippet-archive-cli/config.json`）：

```bash
node apps/cli/dist/cli.js login --email you@example.com --password 'your-password'
```
...
```bash
# 检索（JSON 输出）
node apps/cli/dist/cli.js search --keyword auth --json

# 导出（按协议生成 bundle）
node apps/cli/dist/cli.js export --output ./snippet-bundle.json

# 导入 / 同步（sync 为 import 别名）
node apps/cli/dist/cli.js import --input ./snippet-bundle.json
node apps/cli/dist/cli.js sync --input ./snippet-bundle.json --dry-run
```

4. 常用参数与环境变量：
- API 地址：`--api-base-url` 或 `SNIPPET_ARCHIVE_API_BASE_URL`
- Token：`--token` 或 `SNIPPET_ARCHIVE_TOKEN`
...
2. 在 VS Code 中调试运行插件：
- 打开目录：`apps/vscode-extension`
- 按 `F5` 启动 Extension Development Host
- 在新窗口 `Command Palette`（`Cmd/Ctrl + Shift + P`）执行：
